...
const Login = () => {
  ...
  const handleLoginUser = async (eventObj) => {
    ...
    if (isValidLoginForm) {
      const { isValidResponse, response } = await loginUser(
        inputs,
        setIsLoading
      );
      if (isValidResponse) {
  ...
  return (
    <div className="flex flex-col border border-white rounded-md p-8 w-[300px]">
      ...
      >
        ...
        <button
          className="text-white bg-blue-700 p-2 my-2 flex items-center justify-center"
          type="submit"
        >
